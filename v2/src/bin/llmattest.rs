...
use bountynet::llm_attested::{sha256_prefixed, EventActor};
use bountynet::llm_capture::{
...
) -> Result<bountynet::llm_attested::ContestEvent> {
    let now = now_ms();
...
fn post_event(url: &str, event: &bountynet::llm_attested::ContestEvent) -> Result<()> {
