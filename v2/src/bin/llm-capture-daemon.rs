...
async fn send_event(state: &State, event: &bountynet::llm_attested::ContestEvent) -> Result<()> {
...
async fn send_event_with_backoff(
    state: &State,
    event: &bountynet::llm_attested::ContestEvent,
) -> Result<()> {
...
fn append_event(path: &PathBuf, event: &bountynet::llm_attested::ContestEvent) -> Result<()> {
