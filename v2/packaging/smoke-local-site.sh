...
usage() {
  cat <<'USAGE'
Usage: v2/packaging/smoke-local-site.sh [options] [-- extra local-site args]
...
Examples:
  ...
  v2/packaging/smoke-local-site.sh --mode tee --listen 127.0.0.1:8099
USAGE
}
...
LOCAL_SITE_CMD=(
  "${ROOT_DIR}/v2/packaging/local-site.sh"
  --mode "${MODE}"
  --listen "${LISTEN}"
