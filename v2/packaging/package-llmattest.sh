...
Usage:
  v2/packaging/package-llmattest.sh [--target <triple>] [--version <version>] [--dist <dir>]

Examples:
  v2/packaging/package-llmattest.sh
  v2/packaging/package-llmattest.sh --target x86_64-unknown-linux-gnu
  v2/packaging/package-llmattest.sh --target aarch64-apple-darwin --version 0.1.0
...
if [ -z "$VERSION" ]; then
  VERSION="$(cd "$V2_DIR" && cargo metadata --no-deps --format-version 1 | sed -n 's/.*"version":"\([^"]*\)".*/\1/p' | head -1)"
fi
if [ -z "$VERSION" ]; then
  VERSION="dev"
fi
...
if [ -n "$TARGET" ]; then
  cargo build --manifest-path "$V2_DIR/Cargo.toml" --release --bin llmattest --target "$TARGET"
  TARGET_DIR="$V2_DIR/target/$TARGET/release"
  TARGET_NAME="$TARGET"
else
  cargo build --manifest-path "$V2_DIR/Cargo.toml" --release --bin llmattest
  TARGET_DIR="$V2_DIR/target/release"
...
cp "$V2_DIR/LLM_ATTESTED.md" "$WORK_DIR/$PKG_NAME/LLM_ATTESTED.md"
