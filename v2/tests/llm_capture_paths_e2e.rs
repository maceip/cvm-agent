...
use bountynet::llm_attested::{ContestEvent, CAPTURE_RA_PROFILE};
...
fn wait_runcard_count(fixture: &EventFixture, expected: u64) -> Value {
    let url = format!(
        ...
        fixture.base_url, fixture.event_id, fixture.team_id
    );
    let started = Instant::now();
    while started.elapsed() < Duration::from_secs(5) {
...
fn gateway_path(capture_method: &str, assurance: &str, enforcement: &str, provider: &str) -> Value {
    ...
    if assurance == "attested" {
        let fixture = tdx_stage1_receipt_fixture();
        receipt = fixture.0;
        value_x = fixture.1;
        args.extend([
    ...
    let mut card = wait_runcard_count(&fixture, 1);
    if let Some(receipts_url) = card["share"]["receipts_url"].as_str() {
        let receipts = get_json(&reqwest::blocking::Client::new(), receipts_url);
        if let Some(object) = card.as_object_mut() {
            object.insert("_test_receipts".to_string(), receipts);
        }
    }
    card
}
...
fn attested_gateway_capture_path_e2e() {
    ...
    let receipts = &card["_test_receipts"];
    let receipt = &receipts["receipts"].as_array().unwrap()[0];
    let event_cbor = base64::engine::general_purpose::STANDARD
        .decode(receipt["receipt_cbor_b64"].as_str().unwrap().as_bytes())
        .unwrap();
    let event = ContestEvent::from_cbor(&event_cbor).unwrap();
...
fn capture_daemon_queues_when_sink_down_e2e() {
    ...
    assert!(!std::fs::read_to_string(pending_path).unwrap().is_empty());
}
