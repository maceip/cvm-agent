...
```bash
cd /home/cory/cvms
cargo build --release --bin cvm
file target/release/cvm     # should say ELF 64-bit LSB pie executable
ls -la target/release/cvm    # should be ~13MB
```
...
gcloud compute scp target/release/cvm tdx-test:~/cvm --zone=us-central1-a
...
mkdir -p ~/out
sudo ~/cvm build ~/source --cmd 'echo stage0 build' --output ~/out 2>&1 | tee stage0.log
...
sudo ~/cvm run ~/source --attestation ~/out/attestation.cbor 2>&1 | tee stage1.log &
sleep 3

# 8. In another SSH session, run cmd_check against the local instance
~/cvm check https://127.0.0.1/ 2>&1 | tee check.log
```

**Expected output of `cmd_check`** (ordered lines):

```
[cvm] === attested-TLS check ===
[cvm] Target: 127.0.0.1:443
[cvm] Leaf cert: <~300> bytes DER
[cvm] EAT extension: <~8000+> bytes
[cvm] EAT profile: https://cvm.dev/eat/v2
[cvm] Platform:    Some(Tdx)
[cvm] Value X:     <96 hex chars>
[cvm] SPKI binding:    PASS
[cvm] Verifying platform quote (binding + signature)...
[cvm] Quote binding:   PASS
[cvm] Quote signature: PASS
[cvm]   MRTD: <hex>
[cvm]   RTMR0: <hex>
[cvm]   RTMR1: <hex>
...
[cvm] Chain step 1: verifying previous stage (<N> bytes EAT)
[cvm]   ✓ step 1 quote verifies (Value X stable)
[cvm] Chain:           PASS (1 stage(s) walked)
[cvm] CT (SCTs):       none in cert (self-signed path — expected)
[cvm] Registry:        empty (no entries loaded)
[cvm] === Check Complete ===
[cvm] 127.0.0.1 is a genuine Tdx TEE running Value X <first-16-hex>
```
...
`cvm build → cvm run → cvm check` sequence. Expected
...
az vm create \
  --resource-group cvm-tee-validation \
...
mkdir /sys/kernel/config/tsm/report/cvm-probe: No such device or address
```
...
That proves Azure CVM provisioning, but not cvm's stage chain.
...
#    This packages cvm + source into an EIF image
#    Outputs cvm.eif with PCR0 visible in nitro-cli describe
...
~/cvm check https://<value_x>.aeon.site/ 2>&1 | tee check.log
```
...
`testdata/` (overwriting the old stage-0-only samples):
